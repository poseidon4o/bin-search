...
inline void serialFinishSIMDEytzinger(int c, int needlesCount, int stepCount, const int *bin, const AlignedIntArray &hayStack, const AlignedIntArray &needles, AlignedIntArray &indices) {
...
inline void serialFinishSIMD(int c, int needlesCount, const AlignedIntArray &hayStack, const AlignedIntArray &needles, AlignedIntArray &indices) {
	const int lowCut = hayStack[0];
	const int haystackCount = hayStack.getCount() - 1;
	const int highCut = hayStack[haystackCount];
...
inline void avx256EytzingerRangeCheck(const AlignedIntArray &hayStack, const AlignedIntArray &needles, AlignedIntArray &indices, StackAllocator &allocator) {
	...
	if (useSIMD) {
		...
		while (c + InRangeQueueSize < needlesCount) {
			...
			// produce InRangeQueueSize needles not outside of haystack data range
			// implements if (value < lowCut || value > highCut) { but for InRangeQueueSize values
			while (q < InRangeQueueSize && c < needlesCount) {
				...
				if (candidate >= lowCut && candidate <= highCut) {
					...
					needleIndicesChunks[q] = c; // keep from where this needle is loaded
					q++;
				} else {
					indices[c] = NOT_FOUND;
				}
...
inline void avx256(const AlignedIntArray &hayStack, const AlignedIntArray &needles, AlignedIntArray &indices, StackAllocator &) {
	const int haystackCount = int(hayStack.count);
	const int needlesCount = int(needles.count);
